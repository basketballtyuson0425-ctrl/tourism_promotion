...
現在の提出用デモは、`04_src/frontend/script.js` に初期表示用データを持ち、バックエンドから保存済みYouTube実データを読み込める場合は、その実データを画面に反映している。

YouTube Data API で取得した動画情報はJSONファイルに保存し、動画数、再生数、いいね数、コメント数、タグ情報などを画面表示と集計に使う。
...
現在は、`script.js` 内の初期表示用データと、バックエンドAPIから読み込む保存済みYouTubeデータを組み合わせて表示している。

| データ | 用途 |
|---|---|
| `iseData` | APIから読み込めない場合の伊勢志摩側の初期表示 |
| `miyajimaData` | APIから読み込めない場合の宮島側の初期表示 |
| `targetMarkets` | 米国の対象市場と言語。現在の英語検索語と合わせるため固定値で扱う |
| `ideas.yaml` | 発信案データ |
| `04_src/data/youtube/*.json` | 保存済みYouTube実データ |
| `renderCompareFromSummaries()` | 保存済みデータを使った伊勢志摩と宮島の比較表 |
...
`targetMarkets` は固定値でよい。理由は、現在の検索語が英語中心であり、今回のデモでは米国市場を分析前提として扱うためである。
...
| `title` | 発信案のタイトル | 必須 |
| `summary` | 発信案の説明 | 必須 |
| `targetMarket` | 対象市場。今回のデモでは米国を指定する | 必須 |
...
現在の画面は、初期表示用データを持ちながら、保存済みYouTube実データを優先して反映する構成である。

実データ取得では、まず `search.list` で動画IDを集め、`videos.list` で動画詳細を取得する。今後、コメント本文まで分析する場合は、必要に応じて `commentThreads.list` でコメントを取得する。

最初から全データを大量に集めるのではなく、伊勢志摩と宮島に対象を絞り、英語の旅行投稿を少量から集計するのが現実的である。
...
キーワードページでは、保存済み動画データのタグ情報から頻出キーワードを表示する。

地域比較ページでは、伊勢志摩と宮島の保存済み動画データを使い、動画数、再生数、いいね数、コメント数を比較する。

地域比較ページの課題欄では、伊勢志摩が伸ばすべき点として、再生数の広がり、反応率、旅程の分かりやすさ、独自性の打ち出しを表示する。

発信案ページでは、保存済み動画データのタグ情報を使い、各発信案に関連するYouTube実データ上のキーワードを表示する。

概況ページ右上には、YouTube APIから動画データを取得して保存する操作欄を置く。

地域は伊勢志摩と宮島から選択できる。

API使用量を抑えるため、画面からの取得では6検索語を対象にする。

複数検索語で取得する場合は、YouTube APIの1分あたり制限に当たりにくくするため、検索語ごとに待ち時間を入れる。
